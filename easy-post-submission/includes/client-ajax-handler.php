...
if ( ! class_exists( 'Easy_Post_Submission_Client_Ajax_Handler', false ) ) {
	...
	class Easy_Post_Submission_Client_Ajax_Handler {
		...
		/**
		 * Validates the captcha response based on the configured captcha type.
		 *
		 * This function handles the server-side validation of a captcha response. It checks
		 * the captcha type (reCAPTCHA or Turnstile) and validates accordingly.
		 *
		 * @since 2.4.0
		 *
		 * @param string $captcha_response The response from the captcha widget to be validated.
		 * @param array  $form_settings_result The settings of the form (unused, kept for compatibility).
		 *
		 * @return array An array containing the result of the validation.
		 */
		private function validate_captcha( $captcha_response, $form_settings_result ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed -- Parameter kept for backward compatibility.
			// Check if captcha is enabled globally for forms
			$is_enabled_globally = Easy_Post_Submission_Client_Helper::is_captcha_enabled_for_forms();

			// If not enabled globally, skip verification
			if ( ! $is_enabled_globally ) {
				return [
					'status'  => true,
					'message' => esc_html__( 'Captcha is disabled.', 'easy-post-submission' ),
				];
			}
			...
			if ( 'recaptcha' === $captcha_type ) {
				return $this->validate_recaptcha_response( $captcha_response );
			} elseif ( 'turnstile' === $captcha_type ) {
				return $this->validate_turnstile_response( $captcha_response );
			}

			return [
				'status'  => true,
				'message' => esc_html__( 'No captcha configured.', 'easy-post-submission' ),
			];
		}

		/**
		 * Validates the reCAPTCHA response (legacy wrapper).
		 *
		 * @param string $recaptcha_response The response from the reCAPTCHA widget.
		 * @param array  $form_settings_result The settings of the form (unused).
		 *
		 * @return array An array containing the result of the validation.
		 */
		private function validate_recaptcha( $recaptcha_response, $form_settings_result ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed -- Parameter kept for backward compatibility.
			return $this->validate_captcha( $recaptcha_response, $form_settings_result );
		}

		/**
		 * Validates the reCAPTCHA response against Google's API.
		 *
		 * @since 2.4.0
		 *
		 * @param string $recaptcha_response The response from the reCAPTCHA widget.
		 ...
		 * @return array An array containing the result of the validation.
		 */
		private function validate_recaptcha_response( $recaptcha_response ) {
			...
			if ( ! $global_recaptcha_settings ) {
				$message = current_user_can( 'manage_options' )
					? esc_html__( 'reCAPTCHA is not configured. Please add your reCAPTCHA keys in the Global Captcha Settings.', 'easy-post-submission' )
			...
			if ( is_wp_error( $response ) ) {
				return [
					'status'  => false,
					'message' => esc_html__( 'Unable to verify reCAPTCHA. Please try again.', 'easy-post-submission' ),
				];
			}
		 ...
		 * @since 2.4.0
		 *
		 * @param string $turnstile_response The response from the Turnstile widget.
		 *
		 * @return array An array containing the result of the validation.
		 */
		private function validate_turnstile_response( $turnstile_response ) {
			...
			if ( ! $global_turnstile_settings ) {
				$message = current_user_can( 'manage_options' )
					? esc_html__( 'Turnstile is not configured. Please add your Turnstile keys in the Global Captcha Settings.', 'easy-post-submission' )
					: esc_html__( 'Turnstile verification is not available at this time.', 'easy-post-submission' );
			...
			if ( empty( $turnstile_response ) ) {
				return [
					'status'  => false,
					'message' => esc_html__( 'Turnstile response is missing.', 'easy-post-submission' ),
				];
			}
			...
			if ( is_wp_error( $response ) ) {
				return [
					'status'  => false,
					'message' => esc_html__( 'Unable to verify Turnstile. Please try again.', 'easy-post-submission' ),
				];
			}
			...
			if ( isset( $result->success ) && $result->success ) {
				return [
					'status'  => true,
					'message' => esc_html__( 'Valid Turnstile.', 'easy-post-submission' ),
				];
			} else {
				return [
					'status'  => false,
					'message' => esc_html__( 'Invalid Turnstile verification.', 'easy-post-submission' ),
				];
			}
		}
		...
		private function filter_tags( $tags, $form_settings_result ) {
			// Check if tags are disabled in form settings.
			if ( isset( $form_settings_result['form_fields']['tags']['enable_tags'] ) && false === $form_settings_result['form_fields']['tags']['enable_tags'] ) {
				return [];
			}
		...
		private function try_set_featured_image( $post_id, $form_settings_result, $featured_image_file, $is_remove_featured_image ) {
			...
			// Skip processing when no file was uploaded.
				...
				foreach ( $custom_fields_data as $custom_field_data ) {
					...
					update_post_meta(
						...
						[
							'content'  => $content,
							'label'    => $label,
							'meta_key' => $name, // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key -- This is data storage, not a query.
							'type'     => $type,
						]
					);
				}
		...
		public function get_user_posts() {
			...
			$should_display_post_view = Easy_Post_Submission_Client_Helper::is_post_views_available();
			...
			$args = [
				...
				'posts_per_page' => 10,
				'paged'          => $paged,
				'meta_key'       => 'rbsm_form_id', // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key -- Required to filter posts by form submission.
			...
			if ( $query->have_posts() ) {
				while ( $query->have_posts() ) {
					...
					$post_view = Easy_Post_Submission_Client_Helper::get_post_views( $post_id );
		 ...
		 * @return void
		 */
		public function try_notify_on_post_publish( $post_id, $post_after, $post_before ) {

	...
	class Easy_Post_Submission_Account_Forms {
		...
		public function enqueue_styles() {
			...
			if (
				...
				// Enqueue captcha script if enabled for login or register
				$is_captcha_enabled = false;
				if ( has_shortcode( $post->post_content, 'easy_post_submission_login' ) ) {
					$is_captcha_enabled = Easy_Post_Submission_Client_Helper::is_captcha_enabled_for_login();
				}
				if ( ! $is_captcha_enabled && has_shortcode( $post->post_content, 'easy_post_submission_register' ) ) {
					$is_captcha_enabled = Easy_Post_Submission_Client_Helper::is_captcha_enabled_for_register();
				}

				if ( $is_captcha_enabled ) {
					$captcha_type = Easy_Post_Submission_Client_Helper::get_captcha_type();
					...
					if ( 'recaptcha' === $captcha_type ) {
						...
						$turnstile_settings = Easy_Post_Submission_Client_Helper::get_global_turnstile_settings();
						if ( $turnstile_settings && ! empty( $turnstile_settings['turnstile_site_key'] ) ) {
							wp_enqueue_script(
		...
		/**
		 * Verify captcha response based on the configured captcha type.
		 *
		 * @since 2.4.0
		 *
		 * @param string $captcha_response The captcha response token.
		 * @param string $context The context where captcha is being used (login, register).
		 *
		 * @return bool True if verification passes or captcha is disabled, false otherwise.
		 */
		private function verify_captcha( $captcha_response, $context = 'login' ) {
			// Check if captcha is enabled for this context
			$is_enabled = false;
			if ( 'login' === $context ) {
				$is_enabled = Easy_Post_Submission_Client_Helper::is_captcha_enabled_for_login();
			} elseif ( 'register' === $context ) {
				$is_enabled = Easy_Post_Submission_Client_Helper::is_captcha_enabled_for_register();
			}
			...
			if ( 'recaptcha' === $captcha_type ) {
				return $this->verify_recaptcha_response( $captcha_response );
			} elseif ( 'turnstile' === $captcha_type ) {
				return $this->verify_turnstile_response( $captcha_response );
			}
		...
		/**
		 * Verify reCAPTCHA response (legacy wrapper).
		 *
		...
		private function verify_recaptcha( $recaptcha_response, $context = 'login' ) {
			return $this->verify_captcha( $recaptcha_response, $context );
		}

		/**
		 * Verify reCAPTCHA response against Google's API.
		 *
		 * @since 2.4.0
		 *
		 * @param string $recaptcha_response The reCAPTCHA response token.
		 ...
		 */
		private function verify_recaptcha_response( $recaptcha_response ) {
			...
			if ( ! $global_recaptcha_settings ) {
				$message = current_user_can( 'manage_options' )
						? esc_html__( 'reCAPTCHA is not configured. Please add your reCAPTCHA keys in settings.', 'easy-post-submission' )
						: esc_html__( 'reCAPTCHA verification is not available.', 'easy-post-submission' );
			...
			if ( empty( $recaptcha_response ) ) {
				$this->errors->add( 'recaptcha_missing', esc_html__( 'Please complete the reCAPTCHA verification.', 'easy-post-submission' ) );
			...
			if ( is_wp_error( $response ) ) {
				$this->errors->add( 'recaptcha_error', wp_kses( esc_html__( 'Unable to verify reCAPTCHA. Please try again.', 'easy-post-submission' ), $this->get_allowed_message_html() ) );
			...
			if ( isset( $result->success ) && $result->success ) {
				...
				$this->errors->add( 'recaptcha_invalid', wp_kses( __( 'reCAPTCHA verification failed. Please try again.', 'easy-post-submission' ), $this->get_allowed_message_html() ) );
		 ...
		 * @return bool True if verification passes, false otherwise.
		 */
		private function verify_turnstile_response( $turnstile_response ) {
			// Get global Turnstile settings
			$global_turnstile_settings = Easy_Post_Submission_Client_Helper::get_global_turnstile_settings();

			// If no global settings, show error
			if ( ! $global_turnstile_settings ) {
				$message = current_user_can( 'manage_options' )
						? esc_html__( 'Turnstile is not configured. Please add your Turnstile keys in settings.', 'easy-post-submission' )
						: esc_html__( 'Turnstile verification is not available.', 'easy-post-submission' );
				$this->errors->add( 'turnstile_not_configured', wp_kses( $message, $this->get_allowed_message_html() ) );

				return false;
			}

			// Check if turnstile response is provided
			if ( empty( $turnstile_response ) ) {
				$this->errors->add( 'turnstile_missing', esc_html__( 'Please complete the Turnstile verification.', 'easy-post-submission' ) );

				return false;
			}

			// Use global secret key
			$turnstile_secret_key = $global_turnstile_settings['turnstile_secret_key'] ?? '';
			...
			if ( is_wp_error( $response ) ) {
				$this->errors->add( 'turnstile_error', wp_kses( esc_html__( 'Unable to verify Turnstile. Please try again.', 'easy-post-submission' ), $this->get_allowed_message_html() ) );
			...
			if ( isset( $result->success ) && $result->success ) {
				...
				$this->errors->add( 'turnstile_invalid', wp_kses( __( 'Turnstile verification failed. Please try again.', 'easy-post-submission' ), $this->get_allowed_message_html() ) );
		...
		private function process_login() {
			...
			if ( empty( $credentials['user_login'] ) ) {
				$this->errors->add( 'empty_username', esc_html__( 'The username field is empty.', 'easy-post-submission' ) );
			}

			if ( empty( $credentials['user_password'] ) ) {
				$this->errors->add( 'empty_password', esc_html__( 'The password field is empty.', 'easy-post-submission' ) );
			}
		...
		private function process_register() {
			...
			if ( ! get_option( 'users_can_register' ) ) {
				$this->errors->add( 'registerfail', esc_html__( 'User registration is currently not allowed.', 'easy-post-submission' ) );
		...
		public function register_shortcode( $atts = [] ) {
			...
			if ( ! get_option( 'users_can_register' ) ) {
				return '<div class="rbsm-account-wrapper"><div class="rbsm-account-error">' .
						wp_kses( esc_html__( 'User registration is currently not allowed.', 'easy-post-submission' ), $this->get_allowed_message_html() ) .
						'</div></div>';
			}
			...
			<div class="rbsm-account-wrapper">
				...
				<div class="rbsm-login-form-container">
					<form name="loginform" id="loginform" method="post" action="<?php echo esc_url( add_query_arg( 'action', 'login' ) ); ?>">
						...
						<?php
						// Display captcha if enabled for login
						if ( Easy_Post_Submission_Client_Helper::is_captcha_enabled_for_login() ) {
							$captcha_type = Easy_Post_Submission_Client_Helper::get_captcha_type();
							...
							if ( 'recaptcha' === $captcha_type ) {
								...
								if ( $turnstile_settings && ! empty( $turnstile_settings['turnstile_site_key'] ) ) :
									?>
									<div class="rbsm-turnstile-wrapper">
										<div class="cf-turnstile" data-sitekey="<?php echo esc_attr( $turnstile_settings['turnstile_site_key'] ); ?>"></div>
									</div>
									<?php
							endif;
						}
			...
			<div class="rbsm-account-wrapper">
				...
				<div class="rbsm-register-form-container">
					<form name="registerform" id="registerform" method="post" action="<?php echo esc_url( add_query_arg( 'action', 'register' ) ); ?>">
						...
						<?php
						// Display captcha if enabled for register
						if ( Easy_Post_Submission_Client_Helper::is_captcha_enabled_for_register() ) {
							$captcha_type = Easy_Post_Submission_Client_Helper::get_captcha_type();
